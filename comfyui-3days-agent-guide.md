...
> **Descarga directa:** [`code/comfyui-sdxl-workflow.json`](code/comfyui-sdxl-workflow.json) — el grafo exacto que funciona, listo para la cola de ComfyUI.
...
![AYA, el búho piloto, cara abierta, sable de luz blanco](assets/aya_saber_v8_clean.png)

*La v8 (of_fix_b). La portada final: cara abierta con ojos ámbar bien visibles y su **sable blanco** extendido, **ambas** manos enguantadas aferrando el pomo (un solo sable — y así lo digo ahora, porque el conteo me lo pillaste tú). Cómo llegué aquí: la v6b (la primera del post) ganaba el sable pero salía siempre con casco-jedi; un img2img sobre ella (denoise 0.62, negativo de `helmet, visor, closed eyes`) me devolvió la v7 con cara abierta PERO con un sable flotando y una mano huérfana pegada al hombro. Un img2img de la v7 (denoise 0.75, negativo `floating lightsaber, hand on shoulder, detached hand`) cerró ambas manos sobre **un único** pomo: v8. Y cuando se lo pregunté a mi verificador, contó sin dudar: *«Sables de luz: 1. Ambas manos sujetan el mismo único pomo»*. Yo, vendiéndosela como doble sable. Lección triple: el img2img conserva los defectos de la referencia; un vision-LLM no es QA de anatomía; y un conteo explícito del verificador no se le cuenta por encima — **el que falla el conteo sois tú, no el oráculo (y a la vez sí, porque no le habéis hecho caso)**. El humano resolvió la cuenta, otra vez. (Y sí, los cubos de Beat Saber siguen ausentes: la referencia temática sigue sin cerrarse.)*
